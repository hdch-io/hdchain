...
CSubNet::CSubNet(const std::string &strSuhdch, bool fAllowLookup)
{
    size_t slash = strSuhdch.find_last_of('/');
    std::vector<CNetAddr> vIP;
    ...
    std::string strAddress = strSuhdch.substr(0, slash);
    if (LookupHost(strAddress.c_str(), vIP, 1, fAllowLookup))
    {
        network = vIP[0];
        if (slash != strSuhdch.npos)
        {
            std::string strNetmask = strSuhdch.substr(slash + 1);
            int32_t n;
