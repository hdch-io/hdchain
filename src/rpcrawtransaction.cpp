...
UniValue getrawtransaction(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "           \"hdchaddress\"        (string) hdch address\n"
            "           ,...\n"
            "         ]\n"
            "       }\n"
            "     }\n"
...
UniValue createrawtransaction(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw runtime_error(
            ...
            "    {\n"
            "      \"address\": x.xxx   (numeric or string, required) The key is the hdch address, the numeric value (can be string) is the " + CURRENCY_UNIT + " amount\n"
...
UniValue decoderawtransaction(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "         \"type\" : \"pubkeyhash\",  (string) The type, eg 'pubkeyhash'\n"
            "         \"addresses\" : [           (json array of string)\n"
            "           \"XwnLY9Tf7Zsef8gMGL2fhWA9ZmMjt4KPwg\"   (string) HDCH address\n"
            "           ,...\n"
            "         ]\n"
            "       }\n"
            "     }\n"
...
UniValue crosschainredeem(const UniValue &params, bool fHelp)
{
    ...
    if (!outPutAddress.IsValid())
        return JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Error:Invalid HDCH address");
...
UniValue crosschainrefund(const UniValue &params, bool fHelp)
{
	...
	CBitcoinAddress outPutAddress(CTxDestination(newKey.GetID()));
	if (!outPutAddress.IsValid())
		return JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid HDCH address");
