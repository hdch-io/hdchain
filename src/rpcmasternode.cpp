        ...
        (strCommand != "start" && strCommand != "start-alias" && strCommand != "start-all" && strCommand != "start-missing" &&
         ...
         strCommand != "connect" && strCommand != "outputs" && strCommand != "status"  && strCommand != "license" && strCommand != "payvote" && strCommand != "votelist"))
            throw std::runtime_error(
                ...
                "  start        - Start local Hot masternode configured in hdch.conf\n"
                "  start-alias  - Start single remote masternode by assigned alias configured in hdch.conf\n"
                "  start-<mode> - Start remote masternodes configured in hdch.conf (<mode>: 'all', 'missing', 'disabled')\n"
                "  status       - Print masternode status information\n"
                "  list         - Print list of all known masternodes (see masternodelist for more info)\n"
                "  list-conf    - Print hdch.conf in JSON format\n"
...
UniValue masternodelist(const UniValue& params, bool fHelp)
{
                ...
                && strMode != "check"))
    {
        throw std::runtime_error(
                ...
                "  payee          - Print HDCH address associated with a masternode (can be additionally filtered,\n"
                "                   partial match)\n"
...
UniValue masternodebroadcast(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp  ||
        ...
        throw std::runtime_error(
                ...
                "2. \"passphrase\"     (string, optional) The wallet passphrase\n"
                "\nAvailable commands:\n"
                "  create-alias  - Create single remote masternode broadcast message by assigned alias configured in hdch.conf\n"
                "  create-all    - Create remote masternode broadcast messages for all masternodes configured in hdch.conf\n"
