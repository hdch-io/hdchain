...
bool CNode::IsBanned(CSubNet suhdch)
{
    bool fResult = false;
    {
        LOCK(cs_setBanned);
        banmap_t::iterator i = setBanned.find(suhdch);
        if (i != setBanned.end())
        {
...
bool CNode::IsWhitelistedRange(const CNetAddr &addr) {
    ...
    BOOST_FOREACH(const CSubNet& suhdch, vWhitelistedRange) {
        if (suhdch.Match(addr))
            return true;
    }
    return false;
}

void CNode::AddWhitelistedRange(const CSubNet &suhdch) {
    LOCK(cs_vWhitelistedRange);
    vWhitelistedRange.push_back(suhdch);
}
...
void CNode::copyStats(CNodeStats &stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (HDCH users should be well used to small numbers with many decimal places by now :)
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "HDCH Core " + FormatFullVersion();
...
bool BindListenPort(const CService &addrBind, string& strError, bool fWhitelisted)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. HDCH Core is probably already running."), addrBind.ToString());
        else
...
void DumpBanlist()
{
    ...
    LogPrint("net", "Flushed %d banned node ips/suhdchs to banlist.dat  %dms\n",
