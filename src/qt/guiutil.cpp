...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    // widget->setStyleSheet("QLineEdit { font-size: 11px }");
#if QT_VERSION >= 0x040700
    // We don't want translators to use own addresses in translations
    // and this is the only place, where this address is supplied.
    widget->setPlaceholderText(QObject::tr("Enter a HDCH address (e.g. %1)").arg("ugb46GtdASZqV4vZnLt4acRtJHXWrhpwXi"));
#endif
...
{
    // return if URI is not valid or is no hdch: URI
    if(!uri.isValid() || uri.scheme() != QString("hdch"))
        return false;
...
{
    // Convert hdch:// to hdch:
    //
    //    Cannot handle this later, because hdch:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("hdch://", Qt::CaseInsensitive))
    {
        uri.replace(0, 7, "hdch:");
    }
...
{
    QString ret = QString("HDCH:%1").arg(info.address);
    int paramCount = 0;
...
void openConfigfile()
{
    ...
    /* Open hdch.conf with the associated application */
...
boost::filesystem::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "HDCH.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "HDCH (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("HDCH (%s).lnk", chain);
}
...
boost::filesystem::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "hdch.desktop";
    return GetAutostartDir() / strprintf("hdch-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a hdch.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=HDCH\n";
        else
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the HDCH Core app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add HDCH Core app to startup item list
