...
// Copyright (c) 2016-2018 HDCH Development Team
...
bool CWallet::GetMasternodeVinAndKeys(CTxIn& txinRet, CPubKey& pubKeyRet, CKey& keyRet, std::string strTxHash, std::string strOutputIndex)
{
    ...
    if(strTxHash.empty()) // No output specified, select the one specified by masternodeConfig
    {
        ...
        LogPrintf("CWallet::GetMasternodeVinAndKeys -- Could not locate the masternode configure vin, please check the hdch.conf\n");
        return false;
    }
            ...
            while (true)
            {
                ...
                if (nChange > 0)
                {
                    ...
                    if (nCoinType == ONLY_DENOMINATED) {
                        ...
                        // change transaction isn't always pay-to-hdch-address
                        CScript scriptChange;
