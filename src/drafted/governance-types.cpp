...
GovernanceObjectType GovernanceStringToType(std::string strType) {
    ...
    if(strType == "HDCHNetwork") type = HDCHNetwork;
    if(strType == "HDCHNetworkVariable") type = HDCHNetworkVariable;
...
std::string GovernanceTypeToString(GovernanceObjectType type) {
    ...
    if(type == HDCHNetwork) s = "HDCHNetwork";
    if(type == HDCHNetworkVariable) s = "HDCHNetworkVariable";
