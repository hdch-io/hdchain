...
bool CMasternodeConfig::read(std::string& strErr) {
    ...
    if(masternodeflag)
    {
        ...
        if(alias.empty())
        {
            strErr = _("please add your masternode name into hdch.conf; for example: alias=mynode\n");
            return false;
        }
        ip = GetArg("-externalip", "");
        if(ip.empty())
        {
            strErr = _("Invalid masternode ip, please add your ip into hdch.conf; for example: externalip=0.0.0.0\n");
            return false;
        }
        ...
        if(privKey.empty())
        {
            strErr = _("Invalid masternode privKey, please add your privKey into hdch.conf; for example: masternodeprivkey=***\n");
            return false;
        }
        txHash = GetArg("-collateraloutputtxid", "");
        if(txHash.empty())
        {
            strErr = _("Invalid masternode collateral txid, please add your collateral txid into hdch.conf; for example: collateraloutputtxid=***\n");
            return false;
        }
        ...
        {
            strErr = _("Invalid masternode collateral Index, please add your collateral Index into hdch.conf; for example: collateraloutputindex=0\n");
            return false;
        }
        ...
        if(Params().NetworkIDString() == CBaseChainParams::MAIN) {
            if(port != mainnetDefaultPort) {
                strErr = _("Invalid port detected in hdch.conf") + "\n" +
