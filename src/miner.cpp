...
{
    LogPrintf("HDCHMiner -- started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("hdch-miner");
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("HDCHMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("HDCHMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
            ...
            while (true)
            {
		        ...
		        { 
		            ...
		            {
			            //LogPrintf("HDCHMiner: %d   nExtraNonce: %d\n", pblock->nNonce, nExtraNonce);    
		                break;
                    }
                }
    ...
    {
        LogPrintf("HDCHMiner -- terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("HDCHMiner -- runtime error: %s\n", e.what());
        return;
    }
}
