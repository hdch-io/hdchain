...
//! List of suhdchs to allow RPC connections from
static std::vector<CSubNet> rpc_allow_suhdchs;
...
static bool ClientAllowed(const CNetAddr& netaddr)
{
    if (!netaddr.IsValid())
        return false;
    BOOST_FOREACH (const CSubNet& suhdch, rpc_allow_suhdchs)
        if (suhdch.Match(netaddr))
            return true;
    return false;
}
...
{
    rpc_allow_suhdchs.clear();
    rpc_allow_suhdchs.push_back(CSubNet("127.0.0.0/8")); // always allow IPv4 local suhdch
    rpc_allow_suhdchs.push_back(CSubNet("::1"));         // always allow IPv6 localhost
    if (mapMultiArgs.count("-rpcallowip")) {
        const std::vector<std::string>& vAllow = mapMultiArgs["-rpcallowip"];
        BOOST_FOREACH (std::string strAllow, vAllow) {
            CSubNet suhdch(strAllow);
            if (!suhdch.IsValid()) {
                uiInterface.ThreadSafeMessageBox(
                    strprintf("Invalid -rpcallowip suhdch specification: %s. Valid are a single IP (e.g. 1.2.3.4), a network/netmask (e.g. 1.2.3.4/255.255.255.0) or a network/CIDR (e.g. 1.2.3.4/24).", strAllow),
                    "", CClientUIInterface::MSG_ERROR);
                return false;
            }
            rpc_allow_suhdchs.push_back(suhdch);
        }
    }
    std::string strAllowed;
    BOOST_FOREACH (const CSubNet& suhdch, rpc_allow_suhdchs)
        strAllowed += suhdch.ToString() + " ";
