...
UniValue gobject(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp  ||
        ...
        throw std::runtime_error(
                ...
                "  vote-alias         - Vote on a governance object by masternode alias (using hdch.conf setup)\n"
                "  vote-conf          - Vote on a governance object by masternode configured in hdch.conf\n"
                "  vote-many          - Vote on a governance object by all masternodes (using hdch.conf setup)\n"
                );
    ...
    */
    ...
    if(strCommand == "vote-conf")
    {
        ...
        if(!fMnFound) {
            ...
            resultsObj.push_back(Pair("hdch.conf", statusObj));
