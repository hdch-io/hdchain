...
const char * const BITCOIN_CONF_FILENAME = "hdch.conf";
const char * const BITCOIN_PID_FILENAME = "hdchd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "hdch" is a composite category enabling all HDCH-related debug output
            if(ptrCategory->count(string("hdch"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\HDCH
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\HDCH
    // Mac: ~/Library/Application Support/HDCH
    // Unix: ~/.HDCH
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "HDCH";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/HDCH";
#else
    // Unix
    return pathRet / ".HDCH";
#endif
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty hdch.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override hdch.conf
